...
- **[DEVIATION] Smoke test extended beyond Step 0.7's two-test spec**: Step 0.7 specified exactly two tests: `'opens an empty Isar instance'` and `'clean tearDown removes the temp directory'`. Both tests are present in the final file. A third test, `'openTestIsar injects Isar into IsarService and closeTestIsar resets it'`, was added to verify the `@visibleForTesting` injection seam introduced alongside the IsarService deviation (see entry above). This is an additive change — the two plan-required tests remain intact and the third test provides additional harness confidence.

- **[DEVIATION] Phase branch accumulated 15 commits rather than the 10 projected in Step 10.5**: Step 10.5 anticipated `git log --oneline phase-3/test-foundation ^main` yielding 10 commits (Tasks 0–9). The final history on `main` contains 15 phase-related commits: the original 11 (Tasks 0–10 + fast-forward merge) plus 4 post-Oracle iterative fix commits (`d98996f`, `706590e`, `fa5600d`, `fb1cb90`) that addressed issues discovered during Oracle verification rounds. These extra commits are a natural consequence of the iterative Oracle-review process and do not represent scope creep; they only touch test files, the plan document, `.gitignore`, and `pubspec.yaml` dev_dependencies — all of which are covered by documented deviation entries above.
